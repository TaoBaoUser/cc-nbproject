  ...
  function handleRequest(clientReq, clientRes) {
    ...
    try {
      ...
      sendJson(clientRes, 500, {
        type: 'error',
        error: { type: 'invalid_base_url', message: `供应商「${profile.name}」的 baseUrl 不是合法 URL` },
      });
      return;
    }
    ...
    onRequest({ phase: 'start', method: clientReq.method, path: clientReq.url, profileName: profile.name });

    // 请求体直接 pipe 给上游，不在内存里过一遍。
 ...
 */
async function testUpstream(profile, { timeoutMs = 20000, model = 'claude-3-5-haiku-20241022' } = {}) {
  const startedAt = Date.now();
...
module.exports = { createProxy, testUpstream, createUsageExtractor, filterHeaders };
